<div id="page-wrapper ">
    <div class="container-fluid ">
        <!-- Page Heading -->
        <div class="row " id="main-admin">
            <h1 class="title-exercicios text-center">Gestão de Clientes</h1>
            
            <a class="exercicios-link" href="#">
                <div class="col-md-6 col-lg-4">
                    <div class="exercicios-item text-center">
                        <div class="exercicios-item-header">
                            <i class="fas fa-file-signature fa-5x "></i>
                        </div>
                        <div class="info">
                            <div class="exercicios-title">
                                Ver Clientes
                            </div>
                            <p class="exercicios-descricao"> Visualizar, eliminar clientes associados</p>
                        </div>

                        <div class="bottom">
                            <button class="btn btn-primary exercicios-btn">Ver</button>
                        </div>
                    </div>
                </div>
            </a>

            <a class="exercicios-link" href="#">
                <div class="col-md-6 col-lg-4">
                    <div class="exercicios-item ">
                        <div class="exercicios-item-header">
                            <i class="fas fa-file-signature fa-5x "></i>
                        </div>
                        <div class="info">
                            <div class="exercicios-title">
                                Ver pedidos
                            </div>
                            <p class="exercicios-descricao"> Visualizar pedidos de associação a clientes</p>
                        </div>

                        <div class="bottom">
                            <button class="btn btn-primary exercicios-btn">Ver</button>
                        </div>
                    </div>
                </div>
            </a>
            <a class="exercicios-link" href="#">
                <div class="col-md-6 col-lg-4">
                    <div class="exercicios-item ">
                        <div class="exercicios-item-header">
                            <i class="fas fa-file-signature fa-5x "></i>
                        </div>
                        <div class="info">
                            <div class="exercicios-title">
                                Ver historico
                            </div>
                            <p class="exercicios-descricao"> Visualizar historico de clientes</p>
                        </div>

                        <div class="bottom">
                            <button class="btn btn-primary exercicios-btn">Ver</button>
                        </div>
                    </div>
                </div>
            </a>
              
            
                
        </div>
        <!-- /.row -->
    </div>
    <!-- /.container-fluid -->
</div>
<!-- /#page-wrapper -->
</div><!-- /#wrapper -->